...
const PaymentsPage = () => {
  ...
  const fetchRestaurants = async () => {
    try {
      const response = await fetch('http://192.168.18.187:5000/api/restaurants');
  ...
  const fetchPaymentsByRestaurant = async (restaurantId, name) => {
    try {
      const response = await fetch(`http://192.168.18.187:5000/api/orders/restaurant/${restaurantId}`);
      const data = await response.json();

...
const OfferList = ({ restaurantId: propRestaurantId, isAdmin = false }) => {
  ...
  useEffect(() => {
    if (!propRestaurantId) {
      ...
      axios.get('https://eatster-pro.onrender.com/api/offers').then(res => setOffers(res.data)).catch(() => setOffers([]));
      setAllMode(true);
      setRestaurantId(null);
      setBanners([]);
      setRestaurantName('');
      const fetchBanners = () => {
        axios.get('https://eatster-pro.onrender.com/api/restaurants/banners-all')
          .then(res => {
    ...
    setRestaurantId(storedId);
      axios.get(`https://eatster-pro.onrender.com/api/restaurants/${storedId}/banners`).then(res => {
        setBanners(res.data);
        if (res.data.length > 0) setRestaurantName(res.data[0].restaurant_name);
      });
      axios.get(`https://eatster-pro.onrender.com/api/offers/${storedId}`).then(res => setOffers(res.data)).catch(() => setOffers([]));
      setAllMode(false);
    }
  ...
  const handleAddToCart = async (offer) => {
    ...
    try {
      ...
      if (
        ...
        await axios.post("https://eatster-pro.onrender.com/api/cart/add", {
  ...
  const handleDeleteOffer = async (offerId) => {
    ...
    await axios.delete(`https://eatster-pro.onrender.com/api/offers/${offerId}`, { data: { restaurant_id: restaurantId } });
  ...
  const handleEditOfferSubmit = async (e) => {
    ...
    await axios.put(`https://eatster-pro.onrender.com/api/offers/${editOfferId}`, data);
    setEditOfferId(null);
            ...
            <div key={offer.id} className="bg-white border rounded-xl shadow-md overflow-hidden hover:shadow-xl transition relative">
              {offer.image_url && (
                <div className="w-full h-36 sm:h-40 overflow-hidden flex items-center justify-center bg-gray-50">
                  <img src={`https://eatster-pro.onrender.com${offer.image_url}`} alt={offer.title} className="w-full h-full object-cover rounded-t-xl hover:scale-105 transition-transform duration-300" />
                </div>
              )}

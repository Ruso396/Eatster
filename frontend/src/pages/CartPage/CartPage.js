...
const CartPage = () => {
  ...
  const fetchCartItems = () => {
    if (customerId) {
      axios
        .get(`http://192.168.18.187:5000/api/cart/${customerId}`)
  ...
  const updateQuantity = (cartId, newQty) => {
    ...
    axios
      .put(`http://192.168.18.187:5000/api/cart/update/${cartId}`, { quantity: newQty })
      .then(() => {
        fetchCartItems();
        fetchCartCount();
      })
  ...
  const removeItem = (cartId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`http://192.168.18.187:5000/api/cart/item/${cartId}`)
          .then(() => {

...
const API_BASE_URL = 'http://https://eatster-nine.vercel.app/api/restaurants'; // Your backend API base URL
                ...
                <img
                  src={
                    formData.itemImage
                      ...
                      : `http://https://eatster-nine.vercel.app/${currentEditItem.image_url}`
                  }
                  alt="Preview"
        ...
        {menuItems.length === 0 ? (
          ...
          menuItems.map((item) => (
            <div key={item.id} className="bg-gray-50 p-4 rounded-lg shadow-sm flex items-center space-x-4">
              <img src={item.image_url ? `http://https://eatster-nine.vercel.app/${item.image_url}` : 'https://via.placeholder.com/64'} alt={item.name} className="w-16 h-16 rounded-md object-cover" />
